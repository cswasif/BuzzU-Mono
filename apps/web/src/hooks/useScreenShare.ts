...
export function useScreenShare(): UseScreenShareResult {
  ...
  const prevStatsRef = useRef<{ bytesSent: number; ts: number; framesSent: number } | null>(null);
      ...
      try {
        ...
        stats.forEach((report: any) => {
          if (report.type === 'outbound-rtp' && report.kind === 'video' && report.bytesSent > 0) {
            ...
            if (!outboundVideo || report.bytesSent > outboundVideo.bytesSent) outboundVideo = report;
          }
        ...
        if (outboundVideo?.codecId) {
          stats.forEach((r: any) => { if (r.id === outboundVideo.codecId) codec = r; });
        }
        // Resolve candidates
        if (candidatePair) {
          stats.forEach((r: any) => {
        ...
        const bitrateKbps = prev && elapsed > 0
          ? ((outboundVideo?.bytesSent ?? 0) - prev.bytesSent) * 8 / 1000 / elapsed
          : 0;
        const fpsSent = prev && elapsed > 0
        ...
        console.log(
          ...
          {
            capture: settings ? `${settings.width}x${settings.height}@${settings.frameRate?.toFixed(1)}fps` : 'N/A',
            encoded: outboundVideo ? `${outboundVideo.frameWidth ?? '?'}x${outboundVideo.frameHeight ?? '?'}` : 'N/A',
            ...
            avgEncodeMs: outboundVideo?.framesEncoded
              ? ((outboundVideo.totalEncodeTime / outboundVideo.framesEncoded) * 1000).toFixed(1) + 'ms'
              : 'N/A',
   ...
   */
  const tuneSender = useCallback(async (sender: RTCRtpSender, profile: QualityProfile) => {
    try {
      ...
      // scalabilityMode 'L1T1' — single spatial/temporal layer.
      // Forces hardware encoders (NVENC / AMF / VideoToolbox) to stay
      // in their fast single-pass path instead of SVC.
      (params.encodings[0] as any).scalabilityMode = 'L1T1';

      params.degradationPreference = profile.degradationPreference;

      await sender.setParameters(params);
      console.log('[useScreenShare] Sender tuned:', {
        maxBitrate: profile.maxBitrate,
        maxFramerate: profile.frameRate,
        degradation: profile.degradationPreference,
      });
    } catch (err) {
      console.warn('[useScreenShare] Failed to tune sender:', err);
    }
  }, []);
  ...
  const startScreenShare = useCallback(async (
    ...
    try {
      ...
      if (pc.signalingState === 'closed') {
        console.warn('[useScreenShare] PC closed while picker was open — aborting');
        stream.getTracks().forEach(t => t.stop());
        streamRef.current = null;
        setScreenStream(null);
        throw new Error('RTCPeerConnection closed during screen share picker');
      }
      ...
      if (videoTrack) {
        ...
        videoTrack.addEventListener('ended', () => {
          console.log('[useScreenShare] Video track ended (user stopped via browser UI)');
          // Stale listener from a detached PC — ignore
          if (gen !== pcGenerationRef.current) {
            console.log('[useScreenShare] Stale ended listener (gen', gen, '!= current', pcGenerationRef.current, ') — skipping');
            return;
          }
          // Guard: if stopScreenShare() already ran, skip to avoid double cleanup
          if (cleanupInFlightRef.current) {
            console.log('[useScreenShare] Cleanup already in flight — skipping duplicate');
            return;
          }
          ...
          if (onStoppedCallbackRef.current) {
            try { onStoppedCallbackRef.current(); } catch (_) { /* non-critical */ }
          }
      ...
      if (pc.signalingState === 'stable') {
        ...
        console.warn('[useScreenShare] Deferring renegotiation — signalingState:', pc.signalingState);
      }

      setIsSharing(true);
      console.log('[useScreenShare] Started sharing:', qualityPreset, audioTrack ? '(with audio)' : '(no audio)');
   ...
   */
  const _removeTracksFromPC = useCallback(async (
    pc: RTCPeerConnection,
    sendOffer: (offer: RTCSessionDescriptionInit) => void,
    ...
    try {
      if (videoSender) {
        try { pc.removeTrack(videoSender); } catch (_) { /* already removed */ }
      }
      if (audioSender) {
        try { pc.removeTrack(audioSender); } catch (_) { /* already removed */ }
      }
      ...
      if (pc.signalingState === 'stable') {
        ...
        console.warn('[useScreenShare] Skipping renegotiation — signalingState:', pc.signalingState);
      }
    } catch (err) {
      console.error('[useScreenShare] Failed to remove tracks and renegotiate:', err);
    }
  }, []);
  ...
  const stopScreenShare = useCallback((
    ...
    if (cleanupInFlightRef.current) {
      console.log('[useScreenShare] Cleanup already in flight — skipping duplicate stopScreenShare');
      return;
    }
    cleanupInFlightRef.current = true;
   ...
   */
  const setQuality = useCallback((newQuality: ScreenShareQuality) => {
    ...
    if (videoTrack) {
      ...
      videoTrack.applyConstraints({
        ...
        height: { ideal: profile.height },
        frameRate: { ideal: profile.frameRate },
      }).catch(err => console.warn('[useScreenShare] Failed to apply constraints:', err));
    }
  }, [tuneSender]);
  ...
  const reattachToPC = useCallback(async (
    ...
    if (!stream || !isSharing) {
      console.log('[useScreenShare] reattachToPC: no active capture — skipping');
      return false;
    }
    ...
    if (!videoTrack || videoTrack.readyState !== 'live') {
      console.warn('[useScreenShare] reattachToPC: video track ended — cleaning up');
      _cleanupStream();
      return false;
    }
    ...
    const gen = pcGenerationRef.current;
    videoTrack.addEventListener('ended', () => {
      console.log('[useScreenShare] Video track ended (browser Stop Sharing) — reattached PC context');
      if (gen !== pcGenerationRef.current) {
        console.log('[useScreenShare] Stale ended listener (gen', gen, '!= current', pcGenerationRef.current, ') — skipping');
        return;
      }
    ...
    // ── Audio track (system audio) ───────────────────────────
    const audioTrack = stream.getAudioTracks()[0];
    if (audioTrack && audioTrack.readyState === 'live' && !existingTrackIds.has(audioTrack.id)) {
      audioTrack.contentHint = 'music';
      const sender = pc.addTrack(audioTrack, stream);
      audioSenderRef.current = sender;
      try {
        ...
        console.warn('[useScreenShare] reattach: Failed to tune audio sender:', err);
      }
    }
    ...
    console.log('[useScreenShare] Successfully re-attached screen share to new PC');
    return true;
  }, [isSharing, quality, tuneSender, _cleanupStream, _removeTracksFromPC, startDebugStats]);
