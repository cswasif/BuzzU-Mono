...
const MATCHMAKER_URL = import.meta.env.VITE_MATCHMAKER_URL || 'wss://buzzu-matchmaker.md-wasif-faisal.workers.dev';
...
export function useMatching(): UseMatchingResult {
  ...
  const { peerId, interests, gender, genderFilter, isVerified, joinRoom, avatarSeed } = useSessionStore();
  ...
  const stopMatchingInternal = useCallback(async (cancelQueue: boolean, updateState: boolean, log: boolean) => {
    if (log) console.log('[useMatching] stopMatching, cancelQueue:', cancelQueue);
  ...
  const stopMatching = useCallback(async (cancelQueue: boolean = true) => {
  ...
  const startMatching = useCallback(() => {
    ...
    if (isMatchingRef.current || !peerId) {
      console.log('[useMatching] Cannot start matching: already matching or no peerId.', { isMatchingRef: isMatchingRef.current });
      return;
    }
    ...
    if (wsRef.current) {
      console.log('[useMatching] Closing previous WebSocket before reconnecting');
    ...
    try {
        ...
        const tick = () => {
          ...
          if (now - lastMessageAt > 30000) {
            ...
            if (retryCountRef.current <= maxRetries) {
              const backoffDelay = Math.min(1000 * Math.pow(2, retryCountRef.current - 1), 8000);
              console.warn('[useMatching] No matchmaker response, resending search (attempt', retryCountRef.current, '/', maxRetries, ') with backoff', backoffDelay, 'ms');
              ws.send(messageStr);
              lastMessageAtRef.current = now;
            } else {
              console.warn('[useMatching] No matchmaker response after', maxRetries, 'retries, restarting search');
              stopMatchingInternal(true, true, false);
              const restartDelay = Math.min(1000 * Math.pow(2, consecutiveFailuresRef.current), 16000);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          switch (message.type) {
            case 'Match':
              console.log('[useMatching] Match found! Partner verified?', message.partner_is_verified);
              safeSetMatchData({
              ...
              joinRoom(message.room_id, message.partner_id, message.partner_is_verified || false);
              safeSetIsMatching(false);
              stopMatching(false);
              break;

            case 'Waiting':
              console.log('[useMatching] Waiting in queue, position:', message.position);
      ...
      ws.onerror = (event) => {
        ...
        if (isMountedRef.current && !userInitiatedStopRef.current) {
          consecutiveFailuresRef.current += 1;
          const backoffMs = Math.min(1000 * Math.pow(2, consecutiveFailuresRef.current), 30000)
            + Math.random() * 500; // jitter
          console.log(`[useMatching] Will retry in ${Math.round(backoffMs)}ms (attempt ${consecutiveFailuresRef.current})`);
          safeSetError('Connection to matchmaker failed — retrying…');
          wsRef.current = null;
          if (consecutiveFailuresRef.current <= 5) {
            setTimeout(() => {
              if (isMountedRef.current && !userInitiatedStopRef.current && matchDataRef.current === null) {
                startMatching();
              }
            }, backoffMs);
          } else {
            isMatchingRef.current = false;
            safeSetError('Connection to matchmaker failed after multiple retries');
            safeSetIsMatching(false);
          }
        } else {
          isMatchingRef.current = false;
      ...
      ws.onclose = (event) => {
        ...
        if (event.code === 1000) {
          console.log('[useMatching] Connection closed normally:', event.reason);
          wsRef.current = null;
          ...
          if (wasMatching && !hasMatchData && isMountedRef.current && !wasUserStop) {
            console.log('[useMatching] Restarting matching after normal closure (no match data)');
            setTimeout(() => {
              if (isMountedRef.current && !userInitiatedStopRef.current && matchDataRef.current === null) {
                startMatching();
              }
            ...
            console.log('[useMatching] Not restarting matching - wasMatching:', wasMatching, 'hasMatchData:', hasMatchData, 'isMounted:', isMountedRef.current, 'wasUserStop:', wasUserStop);
          }
          return;
        }
        ...
        if (isMountedRef.current && !wasUserStop && wasMatching) {
          ...
          const backoffMs = Math.min(1000 * Math.pow(2, consecutiveFailuresRef.current), 30000)
            + Math.random() * 500; // jitter
          console.log(`[useMatching] Unexpected close (${event.code}), retrying in ${Math.round(backoffMs)}ms (attempt ${consecutiveFailuresRef.current})`);
          if (consecutiveFailuresRef.current <= 5) {
            safeSetError(`Reconnecting to matchmaker… (attempt ${consecutiveFailuresRef.current})`);
            setTimeout(() => {
              if (isMountedRef.current && !userInitiatedStopRef.current && matchDataRef.current === null) {
                startMatching();
              }
            ...
            safeSetError(`Connection to matchmaker closed (${event.code}: ${event.reason})`);
            safeSetIsMatching(false);
          }
  ...
  }, [peerId, interests, gender, genderFilter, isVerified, joinRoom, stopMatching, safeSetError, safeSetIsMatching, safeSetMatchData, safeSetWaitPosition]);
