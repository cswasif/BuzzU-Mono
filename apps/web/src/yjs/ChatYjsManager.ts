/**
 * ChatYjsManager — Manages Yjs Y.Doc instances for all chat types (DMs, matched, private).
 *
 * Architecture:
 *  - One Y.Doc per chat room (identified by deterministic roomId or matchId)
...
class ChatYjsManagerClass {
    ...
    getOrCreateDoc(roomId: string): Y.Doc {
        ...
        const idb = new IndexeddbPersistence(`buzzu-chat-${roomId}`, doc);
    ...
    addMessage(roomId: string, msg: YjsMessageData): void {
        ...
        doc.transact(() => {
            ...
            map.set(msg.id, msg);
            console.log(`[ChatYjs] ✅ Message added/updated in Y.Map | room=${roomId} id=${msg.id} from=${msg.senderId} mapSize=${map.size}`);
        }, 'local');
    }

    /** Edit a message's content. CRDT handles conflict via last-writer-wins. */
    updateMessageContent(roomId: string, messageId: string, newContent: string): void {
        const doc = this.getOrCreateDoc(roomId);
        doc.transact(() => {
            const map = doc.getMap<YjsMessageData>('messages');
            ...
            if (existing) {
                ...
                console.log(`[ChatYjs] ✏️ Message edited | room=${roomId} id=${messageId}`);
            } else {
                console.warn(`[ChatYjs] ⚠️ Edit failed — message not found | room=${roomId} id=${messageId}`);
            }
        }, 'local');
    }
    ...
    deleteMessage(roomId: string, messageId: string): void {
        ...
        doc.transact(() => {
            ...
            console.log(`[ChatYjs] 🗑️ Message deleted | room=${roomId} id=${messageId} existed=${existed} mapSize=${map.size}`);
        }, 'local');
    }
    ...
    onLocalUpdate(roomId: string, callback: (encodedUpdate: string) => void): () => void {
        ...
        const handler = (update: Uint8Array, origin: any) => {
            if (origin === 'local') {
                ...
                console.log(`[ChatYjs] 📤 Local update to send | room=${roomId} bytes=${update.byteLength} base64len=${encoded.length}`);
                callback(encoded);
            }
        };
    ...
    applyRemoteUpdate(roomId: string, encodedUpdate: string): void {
        ...
        console.log(`[ChatYjs] 📥 Remote update applied | room=${roomId} bytes=${update.byteLength} mapSize=${mapBefore}→${mapAfter}`);
    }
...
/** Singleton Yjs message manager — shared across the app. */
export const ChatYjsManager = new ChatYjsManagerClass();
