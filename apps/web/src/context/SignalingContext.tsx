...
export interface SignalingMessage {
    type: 'RoomStatus' | 'Join' | 'Offer' | 'Answer' | 'IceCandidate' | 'PeerList' | 'Leave' | 'Error' | 'Relay' | 'RelayRequest' | 'RelayResponse' | 'Reachability' | 'Chat' | 'Typing' | 'PublishKeys' | 'RequestKeys' | 'KeysResponse' | 'SignalHandshake' | 'FriendRequest' | 'ScreenShare' | 'VoiceChat' | 'Encrypted' | 'KeyExchange';
    from?: string;
    to?: string;
    room_id?: string;
    peer_id?: string;
...
interface SignalingContextType {
    ...
    error: string | null;
    connect: (roomId: string, currentPeerId: string, options?: { roomType?: string; roomKey?: string; accessKey?: string }) => void;
    disconnect: () => void;
    sendMessage: (message: SignalingMessage) => void;
    onMessage: (type: SignalingMessage['type'], callback: (msg: SignalingMessage) => void) => () => void;
    // Media methods
...
export const SignalingProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const callbacksRef = useRef<Map<string, Set<(msg: SignalingMessage) => void>>>(new Map());
    ...
    const connect = useCallback((roomId: string, currentPeerId: string, options?: { roomType?: string; roomKey?: string; accessKey?: string }) => {
        if (!roomId || !currentPeerId) return;
        // Guard: skip if already connected/connecting to the same room
        if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN && connectedRoomIdRef.current === roomId) return;
        if (isConnectingRef.current && connectedRoomIdRef.current === roomId) return;
        ...
        try {
            ...
            const ws = new WebSocket(`${SIGNALING_URL}/room/${roomId}/websocket?${params.toString()}`);
            wsRef.current = ws;
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    if (message.type === 'PeerList') {
                        ...
                        setPeersInRoom(prev => prev.includes(message.peer_id!) ? prev : [...prev, message.peer_id!]);
                    } else if (message.type === 'Leave' && message.peer_id) {
                        setPeersInRoom(prev => prev.filter(id => id !== message.peer_id));
                    ...
                    if (typeCallbacks) {
                        if (message.type === 'PublishKeys' || message.type === 'RequestKeys' || message.type === 'KeysResponse' || message.type === 'SignalHandshake') {
                            console.log('[SignalingContext] [Signal Debug] Received', message.type, 'from:', message.from, 'to:', message.to);
                        }
            ...
            ws.onclose = (event) => {
                console.log('[SignalingContext] Connection closed', event.code, event.reason);
                isConnectingRef.current = false;
                ...
                if (event.code === 4000) {
                    console.log('[SignalingContext] Stale socket evicted by server — ignoring');
                    return;
                }
                ...
                if ((event.code === 1006 || event.code === 1001) && shouldReconnectRef.current) {
                    console.log('[SignalingContext] Abnormal closure, scheduling reconnect...');
                    setTimeout(() => {
                        if (connectedRoomIdRef.current && shouldReconnectRef.current) {
                            connect(connectedRoomIdRef.current, currentPeerId);
                        }
                    }, 2000);
                }
            };
    ...
    const onMessage = useCallback((type: SignalingMessage['type'], callback: (msg: SignalingMessage) => void) => {
